...
You asked whether detecting sleds at runtime would be easier, and whether
the OS has protected memory yet. It does, and rather thoroughly:

* `enable_nx()` in `boot64.s` sets `EFER.NXE`, and `proc_init` records the
  CPU's real answer in `g_nx_on`. Everything goes through `nx_bit()`,
  which returns 0 when NXE is unavailable — bit 63 is a *reserved* bit
  until NXE is on, and a page with a reserved bit set faults on every
  access including reads, so a hardening flag that bricks the mapping is
  worse than no flag.
* The ELF loader marks any `PT_LOAD` without `PF_X` no-execute, and the
  user stack and `sbrk` heap are `PTE_USER | PTE_WRITE | nx_bit()`.
* `enable_write_protect()` sets `CR0.WP`, so a read-only mapping is
  read-only at CPL 0 too.
* `mm_protect_null()` unmaps page 0.
* `elftest` gates on all of it, including `refused /wx.elf: segment is
  both writable and executable`.

Which means the loader already enforces, at load time, the same rule this
scanner reports as `[WX_SEGMENT]` — and a sled written into the heap or
stack at runtime is non-executable in hardware. Static scanning is still
worth having for files arriving from outside; the runtime side is covered.
